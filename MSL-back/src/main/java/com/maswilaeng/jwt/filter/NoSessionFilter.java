...
import com.maswilaeng.config.CustomUserDetails;
...
@Slf4j
public class NoSessionFilter  extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        ...
        try {
            ...
            log.info("로그인 완료됨: {}", customUserDetails.getUser().getEmail()); // 로그인 정상 완료
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        TokenInfo login = null;
        try {
            login = authService.login(dto);
        } catch (Exception e) {
            e.printStackTrace();
        }

        ResponseCookie accessToken = ResponseCookie
                .from("accessToken", login.getAccessToken())
                .path("/")
                .httpOnly(true)
                // 시간
                .maxAge(jwtTokenProvider.REFRESH_TOKEN_EXPIRE_TIME)
                .sameSite("Lax")
                .build();

        ResponseCookie refreshToken = ResponseCookie
                .from("refreshToken", login.getRefreshToken())
                .path("/api/token")
                .httpOnly(true)
                .maxAge(jwtTokenProvider.REFRESH_TOKEN_EXPIRE_TIME)
                .sameSite("Lax")
